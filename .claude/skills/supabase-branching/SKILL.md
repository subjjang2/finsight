...
정본 마이그레이션은 `supabase/migrations/`(현재 `0001`~`0006`, 4자리 순번). 스키마 규칙은 `CLAUDE.md`
...
- **`supabase/config.toml` 존재 여부**: **현재 레포엔 아직 없다(`supabase init` 미실행)** →
  최초 실행은 반드시 §1부터 한다. 이미 생성돼 있으면 §2로 바로 간다.
...
npx supabase db reset              # 빈 DB에 0001→0006 순서대로 전체 재적용 + seed.sql
```
...
1. **파일 생성** — 기존 4자리 순번을 유지하려면 **수동으로 다음 번호**를 만든다:
   `supabase/migrations/0007_<설명>.sql` (예: `0007_add_budget_table.sql`).
   > `npx supabase migration new`는 타임스탬프 접두사(`20260705..._name.sql`)를 붙여 기존
   > `0001`~`0006` 컨벤션과 섞인다. **순번 유지를 위해 수동 4자리 파일을 권장.**
...
- 마이그레이션 파일은 **4자리 순번(`0007_`, `0008_`…)** 을 유지한다.
