...
### ADR-007: CSV/Excel는 AI 자동 컬럼 매핑, 확인 단계 없이 즉시 분석 (ADR-007a로 개정)
**결정**: 고정 템플릿/카드사 프리셋 대신, 헤더+샘플 몇 행을 Claude에 넘겨 `date/merchant/amount` 컬럼을 자동 매핑한다.
**이유**: 카드사·은행마다 포맷이 제각각 → 어떤 명세서(.csv/.xlsx/.xls)든 받아 UX 마찰 최소화. 언어/인코딩 무관.
**트레이드오프**: 매핑 토큰 비용·오매핑 리스크.

> **ADR-007a (개정, ADR-007의 "사용자 확인" supersede)**: 매핑 확인 UI를 두지 않고 추정값으로 **즉시 분석**한다. 확인 화면은 이탈을 유발해 제거했고, 오매핑은 결과 화면에서 재업로드로 교정한다. 정본: `docs/UX_GUIDE.md` §3, 구현: `components/dashboard/UploadFlow.tsx`(Stage = select/analyzing/done, 매핑 확인 단계 없음).

### ADR-008: 고정 카테고리 enum (12개)
**결정**: AI 자율 생성이 아니라 고정 12개 카테고리 집합으로 분류. 매칭 안 되는 건 `etc`.
**이유**: 전체 이력을 저장하고 월별 추이·비교를 보여주려면 업로드 간 카테고리 명칭이 일관돼야 함.
**트레이드오프**: 세밀함 손실. `etc` 비중 모니터링으로 보완. 정본: `types/category.ts`, `lib/categories.ts`.

### ADR-009: PostHog 제품 분석 (`/ingest` reverse proxy)
**결정**: 제품 분석은 PostHog(US 클라우드). 공개 키만 클라이언트 노출, `next.config.ts`의 `/ingest` reverse proxy로 same-origin 전송. 키 미설정 시 완전 비활성(no-op).
**이유**: 퍼널·리텐션 계측을 자체 구축 없이. same-origin 프록시로 광고차단·서드파티 쿠키 이슈 회피.
**트레이드오프**: 서드파티 데이터 처리자 추가 → PII는 이벤트에 싣지 않고 `identified_only`로 제한.
